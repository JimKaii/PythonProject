...
def weather():
    ...
    reponse = chat_with_chatgpt(
        ...
        '能夠依照台中'f'{city}的周圍,{weather_condition}''天氣來決定拍攝的模式，並且精簡的列出格式如下'
        '先簡單描述一段天氣狀況'
        '是否該前往:'
        '適合的景點:列出三項' 
        '建議的攝影風格:'
        '色調及技術:'
        '其餘的不需要顯示,並幫我用html排版，只要格式，並去除html字樣')
...
def chatbot():
    ...
    bot_reply = chat_with_chatgpt(user_message, 
                                  system_prompt="你是一個專業又有趣的攝影師，"
                                  "任何有關攝影的事情你都知道，從手機、相機、專業相機，都熟知操作及設定，並會推薦攝影構圖。"
                                  "並用簡短的幾行字來描述"
                                  "其餘的不顯示，並幫我用html排版，並去除html字樣")
